        ...
        <div class="panel-collapse collapse <?if($isOpenSection):?>in<?endif?>" id="smt-accordion-primary-item-<?=$arSection['ID']?>">
            <div class="panel-body">
                ...
                <table class="table table-striped table-hover smt-table smt-table_services">
                    ...
                    <tr>
                        <?
                        $countProperties = count($arSection["ITEMS"][0]["PROPERTIES"]);
                        $widthCols = round(100/($countProperties + 3));
                        ?>
                        <th></th>
                        <?if(!empty($arSection["ITEMS"] && $arSection["ITEMS"][0])):?>
                        <?foreach($arSection["ITEMS"][0]["PROPERTIES"] as $arProperty):?>
                        <?if(in_array($arProperty["CODE"], $arParams["PROPERTY_CODE"])):?>
                        <th width="<?=$widthCols?>%"><?=$arProperty["NAME"]?></th>
                        <?endif?>
                        <?endforeach;?>
                        <?endif?>
                    </tr>
