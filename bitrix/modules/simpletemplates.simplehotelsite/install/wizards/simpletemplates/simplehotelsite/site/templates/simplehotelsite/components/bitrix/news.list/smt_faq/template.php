					...
					<div class="smt-blog-item__content">
						<?if($arParams["DISPLAY_PREVIEW_TEXT"] == "Y"):?>
							<div class="smt-blog-item__text">
								<a href="<?=$linkHref?>" class="smt-blog-item__link">
								<?if($arItem["PREVIEW_TEXT_TYPE"] == 'text'):?>
						...
						<?if($arParams["DISPLAY_NAME"] == "Y"):?>
							<div class="smt-blog-item__name">
								<?if($arParams["SMT_LINK_SHOW_MODE"] == "DETAIL_PICTURE" || !$linkHref):?>
									<span class="smt-blog-item__text"><?=$arItem["NAME"]?></span>
								<?else:?>
									<span class="smt-blog-item__text"><?=$arItem["NAME"]?></span>
								<?endif?>
							</div>
						<?endif?>
						<?if(!empty($arItem['PROPERTIES']) && $arParams['MAIN_BLOCK_PROPERTY_CODE']):?>
						...
						<?if($arParams['DISPLAY_BUTTON'] == 'Y'):?>
							<div class="smt-blog-item__button">
								<a href="<?=$linkHref?>" class="btn smt-btn smt-btn-sm"><?=GetMessage("SMT_BCSL_BUTTON_LIST_FAQ")?></a>
							</div>
						<?endif?>
					</div>
					<?if($arParams["LINE_ELEMENT_COUNT"] == 1):?>
