...
<?if(!empty($arResult["ITEMS"])):?>
	...
	<?endif;?>
	<div class="smt-blog-list smt-blog-list_benefit">
		...
		<?foreach($arResult["ITEMS"] as $arItem):?>
			...
			?>
			<?if($arParams["SMT_SLIDER"] == "Y"):?>
			<?elseif($arParams["LINE_ELEMENT_COUNT"] > 1 || $arParams["LINE_ELEMENT_COUNT"] == 1):?>
			...
			<?endif?>
				<div class="smt-blog-item<?if($arParams["LINE_ELEMENT_COUNT"] == 1):?> smt-blog-item_line<?endif?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
					<?if($arParams["LINE_ELEMENT_COUNT"] == 1 || $arParams["LINE_ELEMENT_COUNT"] > 1):?>
					<div class="row">
					<div class="<?=implode(' ', $arResult["LINE_ELEMENT_GRID"][0])?>">
					<?endif?>
					<?
					$linkHref = $arItem["DETAIL_PAGE_URL"];
					$imageLinkClass = array("smt-image");
					...
					<?if($arParams["DISPLAY_PICTURE"] == "Y" && $arItem["PREVIEW_PICTURE"]["SRC"]):?>
						...
						<?endif?>
					<?endif?>
					<?if($arItem["PROPERTIES"]["ICON"]["VALUE"] && !$arItem["PREVIEW_PICTURE"]["SRC"]):?>
						<div class="smt-blog-item__icon smt-color-primary"><span class="<?=$arItem["PROPERTIES"]["ICON"]["VALUE"]?>"></span></div>
					<?endif?>
					<?if($arParams["LINE_ELEMENT_COUNT"] == 1 || $arParams["LINE_ELEMENT_COUNT"] > 1):?>
					</div>
