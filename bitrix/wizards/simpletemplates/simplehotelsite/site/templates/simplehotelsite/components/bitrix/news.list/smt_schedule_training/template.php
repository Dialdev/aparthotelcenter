...
<?if(!empty($arResult["ITEMS"])):?>
    <?$APPLICATION->IncludeFile(
        SITE_DIR."smt_include/training_schedule_header.php",
        Array(),
        Array("MODE"=>"html")
    );?>
	...
	<div class="smt-blog-list">
		...
		<?foreach($arResult["ITEMS"] as $arItem):?>
			...
			?>
            <?
            $arItemElement = array_pop($arItem["DISPLAY_PROPERTIES"]["TRAINING"]["LINK_ELEMENT_VALUE"]);
            ?>
			<?if($arParams["SMT_SLIDER"] == "Y"):?>
			<?elseif($arParams["LINE_ELEMENT_COUNT"] > 1):?>
			...
			<?endif?>
				<div class="smt-blog-item<?if($arParams["LINE_ELEMENT_COUNT"] == 1):?> smt-blog-item_line<?endif?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
					...
					?>
					<?if($arParams["DISPLAY_PICTURE"] == "Y" && $arItemElement["PREVIEW_PICTURE"]["SRC"]):?>
						<?if($linkHref):?>
						<a class="<?=implode(' ', $imageLinkClass)?>" href="<?=$linkHref?>">
						<?else:?>
						<span class="<?=implode(' ', $imageLinkClass)?>">
						<?endif?>
						<img src="<?=$arItemElement["PREVIEW_PICTURE"]["SRC"]?>" class="smt-image__image smt-img-thumbnail">
							...
							<span class="smt-image__over-text">
								<span class="smt-image-text-over smt-image-text-over_bg smt-image-text-over_center">
									<?if($arParams["DISPLAY_IMAGE_NAME"] == "Y"):?>
										<span class="smt-image-text-over__name"><?=$arItemElement["NAME"]?></span>
									<?endif?>
									<?if($arParams["DISPLAY_IMAGE_PREVIEW_TEXT"] == "Y"):?>
										<span class="smt-image-text-over__descr">
										<?if($arItemElement["PREVIEW_TEXT_TYPE"] == 'text'):?>
											<?=$arItemElement["PREVIEW_TEXT"]?>
										<?else:?>
											<?=$arItemElement["PREVIEW_TEXT"]?>
										<?endif?>
										</span>
						...
						<?if($arParams["DISPLAY_NAME"] == "Y"):?>
							<div class="smt-blog-item__name">
								<?if($arParams["SMT_LINK_SHOW_MODE"] == "DETAIL_PICTURE" || !$linkHref):?>
									<span class="smt-blog-item__link"><?=$arItemElement["NAME"]?></span>
								<?else:?>
									<a href="<?=$linkHref?>" class="smt-blog-item__link"><?=$arItemElement["NAME"]?></a>
								<?endif?>
							</div>
						<?endif?>
						<?if(!empty($arItem['PROPERTIES'])):?>
                            <?if(is_array($arParams['MAIN_BLOCK_PROPERTY_CODE']) && !empty($arParams['MAIN_BLOCK_PROPERTY_CODE'])) {?>
							<table class="table">
