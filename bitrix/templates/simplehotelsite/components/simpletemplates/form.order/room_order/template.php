...
<?if ($arParams["MAX_FILE_SIZE"] > 0):?><input type="hidden" name="MAX_FILE_SIZE" value="<?=$arParams["MAX_FILE_SIZE"]?>" /><?endif?>
	<?if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])):?>
		<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
        <?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] == "DATE_IN" ||
             $propertyID == "NAME" ||
             $arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] == "NUM_ADULT"):?>
        <div class="row">
            <div class="col-md-6">
        <?elseif($arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] == "DATE_OUT" ||
                 $arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] == "PHONE" ||
                 $arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] == "NUM_CHILDREN"):?>
            <div class="col-md-6">
        <?else:?>
        <div class="row">
            <div class="col-md-12">
        <?endif?>
			...
			<?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] && $arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] == "L" && $arResult["PROPERTY_LIST_FULL"][$propertyID]["LIST_TYPE"] == "C"):?>
				<?if($label):?>
					<label for="smt-order-form-field-<?=$arParams["FORM_SUFFIX"]?>-<?=strtolower($propertyID)?>">
						<?=$label?>
						<?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?><span class="smt-required-star">*</span><?endif?>
					</label>
				<?endif?>
				<?$checkboxType = $arResult["PROPERTY_LIST_FULL"][$propertyID]["MULTIPLE"] == "Y" ? "checkbox" : "checkbox";?>
