...
<section class="smt-widget smt-widget-benefit">
	<div class="container">
		...
		<?endif?>
			...
			<?foreach($arResult["ITEMS"] as $arItem):?>
				...
				<?if($arParams["SMT_SLIDER"] == "Y"):?>
				<?else:?>
				<div class="smt-widget-benefit__item col-xs-12 col-sm-6 col-md-<?=$rowLineCount?> col-lg-<?=$rowLineCount?>">
				<?endif?>
					<div class="smt-benefit-item" id="<?=$this->GetEditAreaId($arItem['ID']);?>" style="background-image: url(<?=$arItem["DETAIL_PICTURE"]["SRC"]?>)">
						<div class="smt-benefit-item__content">
							...
							<div class="smt-benefit-item__header">
								...
								<?if($arParams["DISPLAY_PREVIEW_TEXT"] == "Y"):?>
									<?if($linkHref):?>
										<a class="h4 smt-header smt-header-underline-center" href="<?=$linkHref?>"><?=$arItem["PREVIEW_TEXT"]?></a>
									<?else:?>
										<div class="smt-benefit-item__header h4 smt-header smt-header-underline-center"><?=($arItem["PREVIEW_TEXT"])?$arItem["PREVIEW_TEXT"]:$arItem["NAME"]?></div>
									<?endif?>
								<?endif;?>
