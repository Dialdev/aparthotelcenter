...
<?$APPLICATION->IncludeFile(
    SITE_DIR."smt_include/training_link_schedule.php",
    Array("arrFilter"=>array("PROPERTY_TRAINING"=>$ElementID)),
    Array("MODE"=>"html")
);?>
<? } ?>
<?
if ($ElementID > 0)
{
    ...
    if($obCache->StartDataCache($CACHE_TIME, $strCacheID, $componentPath))
    {
        ...
        $rsProperties = CIBlockElement::GetProperty($arParams["IBLOCK_ID"], $ElementID, "sort", "asc", array("ACTIVE"=>"Y","CODE"=>"PRICE"));
        while($arProperty = $rsProperties->Fetch())
        {
            if(is_array($arProperty["VALUE"]) && count($arProperty["VALUE"])>0)
            {
                foreach($arProperty["VALUE"] as $value)
                    $arLinkProperty[$value]=$arProperty;
            }
            elseif(!is_array($arProperty["VALUE"]) && strlen($arProperty["VALUE"])>0) {
                $arLinkProperty[$arProperty["VALUE"]]=$arProperty;
            }
        }
    ...
    if(count($arLinkProperty) > 0) {
        ?>
        <?$APPLICATION->IncludeFile(
            SITE_DIR."smt_include/training_link_price.php",
            Array("arrFilter"=>array("SECTION_IDS"=>array_keys($arLinkProperty))),
            Array("MODE"=>"html")
        );?>
    <? } ?>
<? } ?>
