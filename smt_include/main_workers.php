<section class="smt-widget smt-widget_padding smt-widget_border smt-widget-gallery-list">
    <div class="container">
        <?$APPLICATION->IncludeFile(
            SITE_DIR."smt_include/main_workers_header.php",
            Array(),
            Array("MODE"=>"html")
        );?>
        <?$APPLICATION->IncludeComponent(
	"simpletemplates:elements.filter", 
	".default", 
	array(
		"FILTER_NAME" => "galleryFilter",
		"IBLOCK_ID" => "8",
		"IBLOCK_TYPE" => "smt_content",
		"COMPONENT_TEMPLATE" => ".default",
		"PROPERTY_CODE" => array(
			0 => "MAIN",
			1 => "",
		),
		"PROPERTY_VALUES" => array(
			0 => "Да",
		),
		"FIELD_CODE" => array(
			0 => "",
			1 => "",
		),
		"CACHE_TYPE" => "A",
		"CACHE_TIME" => "36000000",
		"CACHE_GROUPS" => "Y"
	),
	false
);?>
        <?$APPLICATION->IncludeComponent("bitrix:news.list", "smt_team", array(
	"DISPLAY_DATE" => "N",
		"DISPLAY_NAME" => "N",
		"DISPLAY_PICTURE" => "Y",
		"DISPLAY_PREVIEW_TEXT" => "N",
		"AJAX_MODE" => "N",
		"IBLOCK_TYPE" => "smt_company",
		"IBLOCK_ID" => "18",
		"NEWS_COUNT" => "8",
		"SORT_BY1" => "SORT",
		"SORT_ORDER1" => "ASC",
		"SORT_BY2" => "SORT",
		"SORT_ORDER2" => "ASC",
		"FILTER_NAME" => "galleryFilter",
		"FIELD_CODE" => array(
			0 => "DETAIL_PICTURE",
			1 => "",
		),
		"PROPERTY_CODE" => array(
			0 => "POSITION",
			1 => "PHONE",
			2 => "EMAIL",
			3 => "LINK",
			4 => "ICON",
			5 => "",
		),
		"CHECK_DATES" => "Y",
		"DETAIL_URL" => "",
		"PREVIEW_TRUNCATE_LEN" => "",
		"ACTIVE_DATE_FORMAT" => "d.m.Y",
		"SET_TITLE" => "N",
		"SET_STATUS_404" => "N",
		"INCLUDE_IBLOCK_INTO_CHAIN" => "N",
		"ADD_SECTIONS_CHAIN" => "N",
		"HIDE_LINK_WHEN_NO_DETAIL" => "N",
		"PARENT_SECTION" => "",
		"PARENT_SECTION_CODE" => "",
		"INCLUDE_SUBSECTIONS" => "N",
		"CACHE_TYPE" => "A",
		"CACHE_TIME" => "36000000",
		"CACHE_FILTER" => "Y",
		"CACHE_GROUPS" => "Y",
		"PAGER_TEMPLATE" => ".default",
		"DISPLAY_TOP_PAGER" => "N",
		"DISPLAY_BOTTOM_PAGER" => "N",
		"PAGER_TITLE" => "",
		"PAGER_SHOW_ALWAYS" => "N",
		"PAGER_DESC_NUMBERING" => "N",
		"PAGER_DESC_NUMBERING_CACHE_TIME" => "36000",
		"PAGER_SHOW_ALL" => "N",
		"AJAX_OPTION_JUMP" => "N",
		"AJAX_OPTION_STYLE" => "N",
		"AJAX_OPTION_HISTORY" => "N",
		"COMPONENT_TEMPLATE" => "smt_team",
		"MAIN_BLOCK_PROPERTY_CODE" => array(
			0 => "POSITION",
			1 => "PHONE",
			2 => "EMAIL",
		),
		"AJAX_OPTION_ADDITIONAL" => "",
		"SET_BROWSER_TITLE" => "N",
		"SET_META_KEYWORDS" => "N",
		"SET_META_DESCRIPTION" => "N",
		"SET_LAST_MODIFIED" => "N",
		"LINE_ELEMENT_COUNT" => "4",
		"DISPLAY_DESCRIPTION" => "N",
		"PICTURE_RESIZE" => "Y",
		"PICTURE_RESIZE_MODE" => "2",
		"PICTURE_RESOLUTION" => "1x1",
		"PICTURE_RESIZE_SOURCE" => "DETAIL_PICTURE",
		"PICTURE_RESIZE_WIDTH" => "240",
		"PICTURE_RESIZE_HEIGHT" => "320",
		"SMT_SLIDER" => "Y",
		"SMT_SLIDER_ITEMS" => "4",
		"SMT_SLIDER_MARGIN" => "",
		"SMT_SLIDER_NAV" => "Y",
		"SMT_SLIDER_DOTS" => "N",
		"SMT_SLIDER_VERTICAL_ALIGN" => "N",
		"SMT_SLIDER_RESPONSIVE" => array(
			0 => "0",
			1 => "480",
			2 => "660",
			3 => "960",
			4 => "",
		),
		"SMT_SLIDER_CUSTOM" => "",
		"PAGER_BASE_LINK_ENABLE" => "N",
		"SHOW_404" => "N",
		"MESSAGE_404" => "",
		"SMT_SLIDER_RESPONSIVE_ITEMS_0" => "1",
		"SMT_SLIDER_RESPONSIVE_CUSTOM_0" => "",
		"SMT_SLIDER_RESPONSIVE_ITEMS_1" => "1",
		"SMT_SLIDER_RESPONSIVE_CUSTOM_1" => "",
		"SMT_SLIDER_RESPONSIVE_ITEMS_2" => "2",
		"SMT_SLIDER_RESPONSIVE_CUSTOM_2" => "",
		"SMT_SLIDER_RESPONSIVE_ITEMS_3" => "4",
		"SMT_SLIDER_RESPONSIVE_CUSTOM_3" => "",
		"SMT_LINK_SHOW_MODE" => "LINK",
		"DISPLAY_IMAGE_NAME" => "Y",
		"DISPLAY_IMAGE_PREVIEW_TEXT" => "Y",
		"DISPLAY_IMAGE_FIXED" => "N",
		"LINE_ELEMENT_GRID" => "12 12 4 4",
		"DISPLAY_BUTTON" => "N"
	),
	false,
	array(
	"ACTIVE_COMPONENT" => "N"
	)
);
        ?>
    </div>
</section>